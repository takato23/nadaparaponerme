...
export type CalendarEventType =
  ...
  | 'workout' // Ejercicio o actividad física
...
// ===================================
// Digital Twin & Virtual Mirror Types
// ===================================

export interface DigitalTwinProfile {
  id: string;
  userId: string;
  sourceImages: string[]; // 3-4 photos uploaded by user
  modelStatus: 'processing' | 'ready' | 'failed';
  createdAt: string;
  // Mocked Nano Banana model reference
  modelId?: string;
}

export type VirtualMirrorContext = 'mirror' | 'street' | 'night_event' | 'beach';

export interface VirtualTryOnRequest {
  twinId: string;
  clothingItemId: string;
  context: VirtualMirrorContext;
}
...
// =====================================================
// Studio Virtual Try-On: Slot System Types
// =====================================================

/**
 * Clothing slots for virtual try-on
 * Each slot represents a body zone that can hold one item
 */
export type ClothingSlot =
  | 'top_base'    // Remera, camisa, musculosa
  | 'top_mid'     // Buzo, sweater, cardigan
  | 'outerwear'   // Campera, tapado, blazer
  | 'bottom'      // Pantalón, falda, short
  | 'one_piece'   // Vestido, enterito (replaces top+bottom)
  | 'shoes'       // Zapatillas, botas, sandalias
  | 'head'        // Gorro, vincha, sombrero
  | 'eyewear'     // Anteojos, lentes de sol
  | 'bag'         // Cartera, mochila, clutch
  | 'hand_acc';   // Reloj, pulsera

/**
 * Mapping of clothing categories to valid slots
 */
export const CATEGORY_TO_SLOT: Record<string, ClothingSlot[]> = {
  'top': ['top_base', 'top_mid'],
  'bottom': ['bottom'],
  'shoes': ['shoes'],
  'outerwear': ['outerwear'],
  'one-piece': ['one_piece'],
  'accessory': ['head', 'eyewear', 'bag', 'hand_acc'],
};
...
export interface SlotConfig {
  ...
  labelShort: string;
  required: boolean;
  layerOrder: number; // Lower = closer to body
  categories: string[]; // Which categories can go here
  icon: string; // Material symbol icon name
}

export const SLOT_CONFIGS: SlotConfig[] = [
  { id: 'top_base', label: 'Top base', labelShort: 'Top', required: true, layerOrder: 1, categories: ['top'], icon: 'checkroom' },
  { id: 'top_mid', label: 'Capa media', labelShort: 'Buzo', required: false, layerOrder: 2, categories: ['top', 'outerwear'], icon: 'layers' },
  { id: 'outerwear', label: 'Abrigo', labelShort: 'Campera', required: false, layerOrder: 3, categories: ['outerwear'], icon: 'ac_unit' },
  { id: 'one_piece', label: 'Enterizo', labelShort: 'Vestido', required: false, layerOrder: 1, categories: ['one-piece'], icon: 'style' },
  { id: 'bottom', label: 'Pantalón/Falda', labelShort: 'Bottom', required: true, layerOrder: 4, categories: ['bottom'], icon: 'straighten' },
  { id: 'shoes', label: 'Calzado', labelShort: 'Zapatos', required: false, layerOrder: 5, categories: ['shoes'], icon: 'steps' },
  { id: 'head', label: 'Cabeza', labelShort: 'Gorro', required: false, layerOrder: 6, categories: ['accessory'], icon: 'face' },
  { id: 'eyewear', label: 'Anteojos', labelShort: 'Lentes', required: false, layerOrder: 7, categories: ['accessory'], icon: 'visibility' },
  { id: 'bag', label: 'Bolso', labelShort: 'Bolso', required: false, layerOrder: 8, categories: ['accessory'], icon: 'shopping_bag' },
  { id: 'hand_acc', label: 'Accesorios mano', labelShort: 'Reloj', required: false, layerOrder: 9, categories: ['accessory'], icon: 'watch' },
];

/**
 * Maximum items per generation to avoid AI confusion
 */
export const MAX_SLOTS_PER_GENERATION = 6;

/**
 * Generation preset options for Virtual Try-On backgrounds
 */
...
export const GENERATION_PRESETS: GenerationPresetConfig[] = [
  {
    id: 'overlay',
    label: 'Original',
    description: 'Mantiene tu fondo',
    promptModifier: 'OVERLAY MODE: Mantén EXACTAMENTE el mismo encuadre, pose, fondo, iluminación y ángulo de la foto del usuario. Solo cambia la ropa, nada más.',
    icon: 'filter_none'
  },
  {
    id: 'mirror_selfie',
    label: 'Espejo',
    description: 'Selfie frente al espejo',
    promptModifier: 'MIRROR SELFIE MODE: Coloca a la persona frente a un espejo de cuerpo completo en un dormitorio o vestidor aesthetic. Simula una selfie de espejo con iluminación natural suave. Incluye el reflejo.',
    icon: 'door_sliding'
  },
  {
    id: 'studio',
    label: 'Studio',
    description: 'Fondo profesional',
    promptModifier: 'STUDIO MODE: Coloca a la persona en un ambiente de estudio fotográfico neutro (fondo gris). Iluminación suave y profesional tipo softbox.',
    icon: 'photo_camera'
  },
  {
    id: 'street',
    ...
    promptModifier: 'STREET MODE: Coloca a la persona en una calle urbana con edificios y ambiente de ciudad. Iluminación natural de día.',
    icon: 'location_city'
  },
  {
    id: 'golden_hour',
    label: 'Atardecer',
    description: 'Luz dorada',
    promptModifier: 'GOLDEN HOUR MODE: Coloca a la persona en exterior con iluminación dorada de atardecer. Tonos cálidos y ambiente romántico.',
    icon: 'wb_twilight'
  },
  {
    id: 'coffee_shop',
    ...
    promptModifier: 'COFFEE SHOP MODE: Coloca a la persona en una cafetería aesthetic con plantas, luces cálidas y ambiente acogedor.',
    icon: 'local_cafe'
  },
  {
    id: 'home',
    label: 'Living',
    description: 'Tu casa',
    promptModifier: 'HOME MODE: Coloca a la persona en un living moderno y aesthetic. Iluminación natural de ventana.',
    icon: 'weekend'
  },
  {
    id: 'editorial',
    label: 'Editorial',
    description: 'Estilo revista',
    promptModifier: 'EDITORIAL MODE: Estilo editorial de revista de moda. Iluminación dramática de estudio con sombras artísticas. Alta calidad como portada de Vogue.',
    icon: 'auto_awesome'
  },
  {
    id: 'minimalist',
    ...
    promptModifier: 'MINIMALIST MODE: Fondo blanco puro limpio. Iluminación suave y uniforme estilo catálogo.',
    icon: 'crop_square'
  },
  {
    id: 'custom',
    label: 'Otro',
    description: 'Escribí tu escena',
    promptModifier: '', // Will be filled dynamically
    icon: 'edit'
  },
];

/**
 * Slot selection state for the Studio
 */
...
/**
 * Generated look - the result of a virtual try-on
 */
...
/**
 * User's look storage limits based on subscription tier
 */
export const LOOK_STORAGE_LIMITS: Record<string, number> = {
  'free': 10,
  'pro': 50,
...
/**
 * Auto-save settings for the Studio
 */
export interface StudioSettings {
  autoSave: boolean;
  defaultPreset: GenerationPreset;

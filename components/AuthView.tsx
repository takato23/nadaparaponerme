...
import { normalizeUsername, signIn, signUp, signInWithGoogle, validateUsername } from '../src/services/authService';
...
const AuthView = ({ onLogin, initialMode = 'login', variant = 'default' }: AuthViewProps) => {
    ...
    const blinkInterval = reducedMotion ? 7200 : 4200;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isLoginView) {
                analytics.trackEvent('login_started', { method: 'email' });
                ...
                analytics.trackEvent('signup_started', { method: 'email' });
                ...
                const fallbackName = name || normalizedUsername || email.split('@')[0];
                await signUp(email, password, fallbackName, normalizedUsername || undefined);
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            analytics.trackEvent(isLoginView ? 'login_started' : 'signup_started', { method: 'google' });
            // Preserve the current route + redirect params so protected-route bounces
            // can land back on the original destination after OAuth completes.
    ...
    const isDarkEye = variant === 'eye';
    const isLightEye = variant === 'light-eye';
    const has3DEye = isDarkEye || isLightEye;
    const isDarkTheme = isDarkEye;
    const isBusy = loading || oauthLoading;

    // We don't need the portal wrapper if it's light-eye, because the parent modal provides the background.
    const portalOverlayClassName = isDarkTheme
    ...
    const cardClassName = isDarkTheme
        ? 'bg-white/6 backdrop-blur-2xl rounded-3xl shadow-[0_18px_60px_rgba(0,0,0,0.65)] border border-white/12 p-6 text-center text-white sm:p-8'
        : isLightEye
            ? 'w-full text-center' // No background, just content
    ...
    const submitClassName = isDarkTheme
    ...
    const googleButtonClassName = isDarkTheme
    ...
    const toggleClassName = isDarkTheme ? 'text-sm text-white/70 hover:text-white transition-colors' : 'text-sm text-gray-500 hover:text-black transition-colors';
    const dividerTextClassName = isDarkTheme ? 'text-white/50' : 'text-gray-400';
    const dividerLineClassName = isDarkTheme ? 'bg-white/15' : 'bg-gray-200/80';

    return (
        <div
            className={`relative flex h-full w-full min-h-0 justify-center overflow-hidden ${isDarkTheme ? 'items-center text-white' : 'items-center'}`}
        ...
        >
            ...
            <div className={`relative z-40 flex w-full min-h-0 items-center ${isDarkTheme ? 'max-w-md px-5 py-8 sm:px-8' : isLightEye ? 'max-w-md py-4' : 'max-w-md p-5 sm:p-8'}`}>
                <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: 0.5 }}
                    className={`${cardClassName} mx-auto block w-full`}
                >
                    <div className={`flex flex-col items-center ${has3DEye ? 'mb-5' : 'mb-7'}`}>
                        {has3DEye ? (
                            <div className="mb-4 w-full">
                                <div className="relative mx-auto h-24 w-full max-w-[14rem] sm:h-28 sm:max-w-[16rem]">
                                    {canRenderEye3D ? (
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center">
                                            <OjoDeLocaLogo className="h-12 w-12 text-white/80" />
                                        </div>
                                    )}
                                </div>
                            </div>
                        ) : (
                            <OjoDeLocaLogo className="mb-4 h-16 w-16 text-black" />
                        )}
                        <h1 className={`text-[1.75rem] font-bold ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>
                            {isLoginView ? 'Bienvenido' : 'Crear Cuenta'}
                        </h1>
                        <p className={`${isDarkTheme ? 'text-white/70' : 'text-gray-500'} mt-2 text-sm`}>
                            {has3DEye ? 'El ojo sigue tu cursor y te abre la puerta al armario digital.' : 'Tu laboratorio de estilo te espera.'}
                        </p>
                    </div>

-- Fix ambiguity in anti-abuse RPCs introduced by RETURNS TABLE output names
-- `blocked_until` output column shadowed table column references in PL/pgSQL.
...
BEGIN
  ...
  SELECT arl.window_start, arl.window_count, arl.blocked_until
  INTO v_window_start, v_window_count, v_blocked_until
  FROM public.ai_request_limits AS arl
  WHERE arl.user_id = p_user_id AND arl.feature = p_feature
  FOR UPDATE;
  ...
  UPDATE public.ai_request_limits AS arl
  SET
...
BEGIN
  ...
  SELECT arl.error_count, arl.error_window_start, arl.blocked_until
